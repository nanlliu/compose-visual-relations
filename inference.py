...
def edit_images(im, model, labels, num_steps, step_lr, clip, clip_all, device):
	...
	videos = [img_as_ubyte(im.detach().cpu().numpy().transpose((0, 2, 3, 1))[0])]  # GIF
	...
	for i in range(num_steps):
		...
		videos.append(img_as_ubyte(im.detach().cpu().numpy().transpose((0, 2, 3, 1))[0]))
...
def image_inference(
	...
	collate_fn = None
	if clip_all:
		collate_fn = val_dataset.clip_all_collate_fn
	elif clip:
		collate_fn = val_dataset.clip_collate_fn

	# shuffle = False --> make sure each image is generated based on label's ordering
	dataloader = DataLoader(
		dataset=val_dataset, shuffle=False, drop_last=False, batch_size=batch_size, collate_fn=collate_fn
	)

	# create output folder
	image_output_path = os.path.join(output_folder, model_name, f'num_rel_{num_rels}')
	os.makedirs(image_output_path, exist_ok=True)
	# create an output file that records captions
	caption_file_path = os.path.join(output_folder, model_name, f'num_rel_{num_rels}.txt')
	text_file = open(caption_file_path, 'w')

	for i, (ims, labels, captions) in enumerate(tqdm(dataloader)):
		if mode == 'generation':
			results = gen_images(
				model=model, dataset=FLAGS.dataset, labels=labels, num_steps=num_steps, clip=clip, clip_all=clip_all,
				step_lr=FLAGS.step_lr, im_size=FLAGS.im_size, batch_size=ims.shape[0], device=device
			)
		elif mode == 'editing':
			results = edit_images(
		...
		results = (results.detach().cpu().permute(0, 2, 3, 1) * 255.).numpy().astype(np.uint8)
		# save each image
		for j in range(ims.shape[0]):
			file_path = os.path.join(image_output_path, '{:06}.png'.format(i * batch_size + j))
			im = Image.fromarray(results[j])
			im.save(file_path)
			# record the caption
			text_file.write(str(i * batch_size + j) + '\n')
			text_file.write(captions[j] + '\n' * 2)
